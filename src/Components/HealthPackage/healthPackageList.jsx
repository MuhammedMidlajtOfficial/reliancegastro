...
export default function HealthList() {
  ...
  const columns = [
    ...
    { name: 'Duration', selector: row => row.duration, sortable: true },
    {
      ...
      cell: (row) => (
        <>
          <button onClick={() => handleEditClick(row)} style={{
                marginRight: '10px',
                padding: '5px 10px',
              ...
              onMouseLeave={(e) => e.target.style.backgroundColor = '#6fbf73'}>Edit</button>
          <button onClick={() => handleDelete(row._id)} style={{
              ...
              onMouseLeave={(e) => e.target.style.backgroundColor = '#ff6961'}>Delete</button>
        </>
      ),
    },
  ];
